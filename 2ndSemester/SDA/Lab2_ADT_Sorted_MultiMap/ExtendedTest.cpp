...
void testRemoveKey(){
    cout<<"Test remove key - LAB REQUIREMENT\n";
    SortedMultiMap map = SortedMultiMap(asc);
    map.add(1, 1);
    map.add(1, 2);
    map.add(1, 3);
    map.add(2, 10);
    ...
    assert(map.size() == 7);
    vector<TValue> resultVector = map.removeKey(2);
    assert(resultVector.at(0) == 10); // check the returned vector
    assert(resultVector.at(1) == 20);
    assert(resultVector.size() == 2);

    assert(map.search(2).size() == 0); // check it is deleted from the map
    assert(map.search(2).empty() == true);

    SMMIterator it = map.iterator(); // iterate the map and check 1 -> 3 (instead of 1 -> 2 -> 3)
    it.next();it.next();
    assert(it.getCurrent().first == 1 && it.getCurrent().second == 3);
    it.next();
    assert(it.getCurrent().first == 3 && it.getCurrent().second == 1);

    resultVector = map.removeKey(5); //try to remove a nonexistent key
