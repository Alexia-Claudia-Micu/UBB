...
void testSetElemsOnLine(){
    cout<<"Test Set Elements On Line !!! LAB REQUIREMENT\n";
    Matrix m(10,10);
    //assert that the line is Null before using the function on it
    for(int i = 0; i < 10; i++)
        assert(m.element(4, i) == NULL_TELEM);

    m.setElemsOnLine(4, 10);

    // test that the line has been initialised
    for(int i = 0; i < 10; i++)
        assert(m.element(4, i) == 10);
    // test that the rest of the lines are not changed
    for(int i = 0; i < 10; i++)
        if(i != 4)
            for(int j = 0; j < 10; j++)
                assert(m.element(i,j) == NULL_TELEM);
    // test the exception requirement
    try {
        m.setElemsOnLine(-1, 10);
        assert(false);
    }
    catch (exception&) {
        assert(true);
