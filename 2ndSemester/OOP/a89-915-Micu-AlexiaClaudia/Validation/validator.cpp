...
#include "assert.h"
using namespace std;
bool FakeRepository::addReturnValue = true;

void Validator::test_add_ValidEvent_ReturnsTrue() {
    FileRepository fileRepository("");


    auto *fakeRepositoryThatAlwaysReturnsTrue = new FakeRepository();
    fakeRepositoryThatAlwaysReturnsTrue->setAddReturnValue(true);
    AdministratorService administratorService = AdministratorService(fakeRepositoryThatAlwaysReturnsTrue, fileRepository, "html");
    assert(administratorService.add("","",1,1,1,1,1,1,""));
}

void Validator::test_add_ValidEvent_ReturnsFalse() {
    FileRepository fileRepository("");


    auto *fakeRepositoryThatAlwaysReturnsFalse = new FakeRepository();
    fakeRepositoryThatAlwaysReturnsFalse->setAddReturnValue(false);
    AdministratorService administratorService = AdministratorService(fakeRepositoryThatAlwaysReturnsFalse, fileRepository, "html");
    assert(!administratorService.add("","",1,1,1,1,1,1,""));
}
