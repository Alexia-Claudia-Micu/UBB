...
class Ui():
    ...
    def menu(self):
        ...
        while command != "0":
            try:
                ...
                print("13. Check if graph is DAG.")
                print("--------------------------------------------------------------------")
                ...
                if command == "1":
                    ...
                    result = self._service.practical_work_4(self._read_activities())
                    if result == "Graph is not DAG":
                        print(result)
                    else:
                        print(f"Earliest: {str(result[0])}\nLatest: {str(result[1])}\nCritical: {str(result[2])}\n")
    ...
    def _read_activities(self):
        self._service.clear()
        fin = open("activities.txt", "rt")
        duration = {}

        nrVertices = int(fin.readline())
        for i in range(nrVertices):
            line = fin.readline()
            line = line.split(" ")
            vertex = int(line[0])
            if vertex not in self._service.parse():
                self._service.add_vertex(vertex)
            durationOfVertex = int(line[len(line) - 1])
            duration[vertex] = durationOfVertex
            for j in range(1, len(line) - 1):
                adjacent_vertex = int(line[j])
                if adjacent_vertex != -1:
                    if adjacent_vertex not in self._service.parse():
                        self._service.add_vertex(adjacent_vertex)
                    self._service.add_edge(adjacent_vertex, vertex, 1)
