    ...
    def get_production_number_from_grammar(self, state: State) -> int or None:
        for prod in self.grammar.P.keys():
            for prod_value in self.grammar.P[prod]:
                # Ensure prod_value has at least two elements: the production and the production ID
                if len(prod_value) < 2:
                    continue  # Skip invalid production entries

                # Check if the lhs and rhs match the current closure item
                if state.closure[0].lhs == prod and state.closure[0].rhs == prod_value[0]:
                    return prod_value[1]  # Return the production ID
